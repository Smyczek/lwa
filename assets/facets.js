...
class PriceRange extends HTMLElement {
  ...
  setMinAndMaxValues() {
    const progress = this.querySelector('.slider .progress');
    ...
    inputs.forEach(input =>{
      input.addEventListener("input", ()=>{
        progress.style.left = (minInput.value / inputs[0].max) * 100 + "%";
        progress.style.right = 100 - (maxInput.value / inputs[1].max) * 100 + "%";
      })
    })
  }
